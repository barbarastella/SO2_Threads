...
public class Banheiro {
    ...
    public synchronized void entrar(Pessoa p) throws InterruptedException {
       ...
       System.out.println("- Pessoa [" + p.getSexo() + "]: " + p.getId() + " tentando entrar");
        ...
        do {
            if (!this.ocupado && this.tipoOcupacao.equals(p.getSexo())) {
                ...
                if ((ocupacao < capacidade) && tipoOcupacao == p.getSexo() && !this.trava) {
                    ocupacao++;
                    this.cont++;
    ...
    public synchronized void sair(Pessoa p) {
        ...
        if (ocupacao == 0) {
            ...
            // garantir troca de sexo ao esvaziar
            
            if (this.tipoOcupacao.equals("F"))
                this.tipoOcupacao = "M";
